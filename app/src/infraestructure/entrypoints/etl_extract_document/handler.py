...
AWS_STATE_MACHINE_ARN = os.getenv("AWS_S3_BUCKET_NAME",
                                  "arn:aws:states:us-east-1:627912843016:stateMachine:sfn-io-ipkn-kno-exchange-mngt-etl_process-00")
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        logger.info("Iniciando extract document ", extra={"correlation_id": request_id})

        body = json.loads(event.get("body") or "{}")
        page_id = body.get("page_id")
        object_key = body.get("object_key")

        if not page_id or not object_key:
            raise ValueError("Required parameters are missing: page_id or object_key")
